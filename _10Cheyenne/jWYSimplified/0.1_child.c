...
int IDF_Coupling = 0; //0, offline; 1, waste; 2, waste + surface, 3, waste + surface + LWR.
// if (IDF_Coupling == 3) {
...
char *epw_name = "USA_WY_Laramie-General.Brees.Field.725645_TMY3.epw";
...
void receiveLongLat(void) {
    ...
    sprintf(centroidPath, "./%s/%s", resour_name, _centroidsName);
...
void assignGeoData(int currentRank) {
    ...
    sprintf(surfilePath, "./%s/%s", resour_name, _surfName);
...
int main(int argc, char** argv) {
    sprintf(glade_folder_name, "/glade/scratch/lichenwu/%s", _scratch);
    sprintf(resour_name, "%s", _resource);
