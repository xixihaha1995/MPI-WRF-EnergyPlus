...
char *glade_folder_name = "/glade/scratch/lichenwu/TMY3_LA_IDFs38_ep_temp";
...
void receiveLongLat(void) {
    ...
    FILE *file = fopen("./la-resources-22-23-72hrs500m/centroid.csv", "r");
    if (file == NULL) {
...
void assignGeoData(int currentRank) {
    FILE *file = fopen("./la-resources-22-23-72hrs500m/surfaceNames.txt", "r");
    if (file == NULL) {
...
int main(int argc, char** argv) {
    ...
    sprintf(output_path, "%s/saved_%s_ep_trivial_%d", base_path,
            ...
            rank + 1);
    sprintf(idfFilePath, "./la-resources-22-23-72hrs500m/in_uwyo_%d.idf",  rank + 1);
    char* weather_file_path = "./la-resources-22-23-72hrs500m/USA_CA_Hawthorne-Jack.Northrop.Field.722956_TMY3.epw";
