...
SurfaceHandles getSurHandle(EnergyPlusState state, GeoUWyo geoUWyo) {
    ...
    for (int i = 0; i < midLen; i++) {
        ...
        if (surHandles.midHandle[i] < 0) {
            printf("Rank %d: Error: surHandles.midHandle[%d] = %d\n", rank, i, surHandles.midHandle[i]);
            exit(1);
        }
    }
    return surHandles;
...
int main(int argc, char** argv) {
    ...
    midLen = sizeof(geoUWyoMyRank.mid) / sizeof(geoUWyoMyRank.mid[0]);
